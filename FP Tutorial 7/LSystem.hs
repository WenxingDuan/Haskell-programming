...
-- Colors
data Pen
  = Colour GL.GLfloat GL.GLfloat GL.GLfloat
  | Inkless
  deriving (Eq, Ord, Show)
...
toVertex (Pnt x y) =
  GL.vertex $ GL.Vertex3 (realToFrac x) (realToFrac y) (0 :: GL.GLfloat)
...
tree x = f x
  where
    ...
    f x =
      g (x - 1) :#:
      Branch (n :#: f (x - 1)) :#:
      Branch (p :#: f (x - 1)) :#: Branch (g (x - 1) :#: f (x - 1))
...
instance Arbitrary Command where
  arbitrary = sized cmd
    where
      cmd n
        | n <= 0 = oneof [liftM (Go . abs) arbitrary, liftM Turn arbitrary]
