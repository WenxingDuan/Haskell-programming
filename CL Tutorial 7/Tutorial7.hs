...
wffToForm = canonical . toForm . toCNFList
  where
    toForm :: [[Wff a]] -> Form a
    toForm a = And [toClause b | b <- a]
    toClause a = Or [toLit b | b <- a]
    toLit (Not (V a)) = N a
    toLit (V a)       = P a
    toLit _           = error "expected Literal"

prop_form_equiv :: Wff Atom -> Bool
prop_form_equiv wff = (length (dpll (wffToForm wff)) /= 0) == satisfiable wff
...
tseytinToForm p =
  let tt :: Eq a => Wff a -> [Clause (Wff a)]
      ...
      tt r@(a :&: b) =
        [Or [P r, N a, N b], Or [N r, P a], Or [N r, P b]] ++ tt a ++ tt b
      tt r@(a :|: b) =
        [Or [N a, P r], Or [N b, P r], Or [N r, P a, P b]] ++ tt a ++ tt b
      tt r@(a :->: b) =
        [Or [N a, N r, P b], Or [N b, P r], Or [P a, P r]] ++ tt a ++ tt b
      tt r@(a :<->: b) =
        [ Or [N a, N b, P r]
        , Or [N a, N r, P b]
        , Or [N a, P a, P r]
...
prop_tseytin_equiv wff = (length (dpll (tseytinToForm wff)) /= 0) == satisfiable wff
...
atoms = univ [] [] id (\/) (\/) pure -- (\x -> [x])
        -- represent sets as ordered lists \/ is union
  where
    (\/) :: Ord a => [a] -> [a] -> [a]
    [] \/ ys = ys
    xs \/ [] = xs
...
canonical (And cs) =
  (And . uniqueSort) $ map (\(Or xs) -> Or (uniqueSort xs)) cs
...
dpll (And css) =
  let models [] = [[]]
   ...
   in models css
  where
    prioritise = minimumBy (comparing (\(Or lits) -> length lits))
  ...
  where
    ...
    showsPrec p (a :&: b) =
      showParen
        (p > 3)
        (showsPrec 3 a .
         showSpace . showChar '\x2227' . showSpace . showsPrec 3 b)
    showsPrec p (a :|: b) =
      showParen
        (p > 2)
        (showsPrec 2 a .
         showSpace . showChar '\x2228' . showSpace . showsPrec 2 b)
    showsPrec p (a :->: b) =
      showParen
        (p > 1)
        (showsPrec 1 a .
         showSpace . showChar '\x2B62' . showSpace . showsPrec 2 b)
    showsPrec p (a :<->: b) =
      showParen
        (p > 0)
        (showsPrec 0 a .
         showSpace . showChar '\x2B64' . showSpace . showsPrec 0 b)
    showsPrec _ (Not a) = showChar '\x00AC' . showsPrec 11 a
    showString :: String -> String -> String
    showString = (++)
    showChar :: Char -> String -> String
    showChar = (:)
