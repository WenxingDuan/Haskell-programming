...
import Data.List (sortOn, delete, intercalate, intersperse, minimumBy)
...
adjacentCitiesNotSameColour paint =
  and[ or [not (paint a c), not (paint b c)]
...
formalAdjacentCitiesNotSameColour =
  And[ Or [N (Paint a c), N (Paint b c)]
     | a <- cities, b <- cities, adj a b, c <- colours ]
...
dpll (And cs) =
  ...
  in
   models (canonical cs)
  where prioritise = sortOn (\(Or cs) -> length cs)

-- speedy :: Ord a => Form a -> [Val a]
-- speedy (And cs) =
--   let models []           = [[]] -- no clauses; trivial solution
--       models css@(c : cs) = case prioritise c cs of
--         Or []      -> []       -- empty clause; no models
--         Or [x]     -> [ x : m | m <- models $ css << x ]
--         Or (x : _) ->
--           [     x : m | m <- models $ css << x   ]
--           ++
...
--   in
--    models (canonical cs)
--   where lth (Or lits) = length lits
--         prioritise (Or[]) _    = Or[]
--         prioritise c      []   = c
--         prioritise c (c' : cs) =
--           prioritise (if lth c' < lth c then c' else c ) cs
...
speedy (And css) =
  ...
  where prioritise = minimumBy (comparing (\(Or lits) -> length lits))
    -- pri (Or xs) (Or ys) = if length ys < length xs then Or ys else Or xs
    -- prioritise c cs =
    --   foldr pri c cs : cs
